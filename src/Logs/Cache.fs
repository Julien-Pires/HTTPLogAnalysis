...
/// <summary>Represents a set of action that can be performed on a cache</summary>
type CacheAction =
    | Add of Request
...
/// <summary>Represents the data of a cache</summary>
type CacheData = {
...
type RequestCache(lifetime) =
    /// <summary>Adds a new request to the cache</summary>
    ...
    /// <summary>Gets all requests for a specific datetime</summary>
    let getFrame time (frames : Map<int64, Request list>) =
        match frames.TryFind time with
        | Some x -> x
        | None -> []
    
    /// <summary>Gets all requests for datetime range represented by a start date and an end date</summary>
    ...
    /// <summary>Clears items for which lifetime has expired</summary>
    ...
    /// <summary>Represents an agent that process asynchronously cache action</summary>
    ...
    /// <summary>Represents an async action that regularly check for requests lifetime</summary>
    let clearCache =
        let rec loop () = async {
            do! Async.Sleep 1000
            agent.Post Clear
    ...
    /// <summary>Adds a request to the cache</summary>
...
/// <summary>Contains helper methods to work with a cache</summary>
module RequestCache =
    /// <summary>Gets all requests for the specified datetime</summary>
    let getRequestsAt (dateTime : DateTime) (cache : RequestCache) =
        cache.GetFrame <| dateTime.ToTimestamp()
    
    /// <summary>Gets all requests for the specified range of date starting from now</summary>
